...
export class VPCStack extends Stack {
  ...
  constructor(scope: App, id: string, props?: StackProps) {
    ...
    const publicSubnets = [...Array(subnetCount)].map((_: undefined, index: number) => {
      const publicSubnetCIDR = new CfnParameter(this, `PublicSubnetCIDR-${index}`, {
        type: 'String',
        description: 'VPC CIDR.recommend /24',
        default: `${vpcCIDRs.subnets[index].public}`,
      })

      return new CfnSubnet(this, `MyPublicSubnet${index}`, {
        vpcId: vpc.ref,
        cidrBlock: publicSubnetCIDR.valueAsString,
    ...
    const privateSubnets = [...Array(subnetCount)].map((_: undefined, index: number) => {
      const privateSubnetCIDR = new CfnParameter(this, `PrivateSubnetCIDR-${index}`, {
        type: 'String',
        description: 'VPC CIDR.recommend /24',
        default: `${vpcCIDRs.subnets[index].private}`,
      })
      return new CfnSubnet(this, `MyPrivateSubnet${index}`, {
        vpcId: vpc.ref,
        cidrBlock: privateSubnetCIDR.valueAsString,
        availabilityZone: availabilityZones[index],
